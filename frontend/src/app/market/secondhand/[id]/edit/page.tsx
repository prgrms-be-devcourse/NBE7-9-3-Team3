...
export default function SecondhandEditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.resultCode.startsWith('200')) {
        alert('게시글이 수정되었습니다!');
        router.push(`/market/secondhand/${tradeId}`);
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <div className="mb-6">
        <Link href={`/market/secondhand/${tradeId}`} className="text-blue-500 hover:underline">
          ← 뒤로가기
        </Link>
      </div>

      <h1 className="text-3xl font-bold mb-8">중고거래 글 수정</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4 pt-4">
          ...
          <Link
            href={`/market/secondhand/${tradeId}`}
