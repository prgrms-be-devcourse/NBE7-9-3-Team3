...
export default function FishDetailPage() {
  ...
  const buildReturnUrl = () => {
    ...
    return `/market/fish?${params.toString()}`;
  };
  ...
  const fetchComments = async () => {
    try {
      const data: ApiResponse<TradeComment[]> = await fetchApi(`/api/market/fish/${tradeId}/comments`);
      if (data.resultCode.startsWith('200')) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetchApi(`/api/market/fish/${tradeId}`, { method: 'DELETE' });
      alert('게시글이 삭제되었습니다.');
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchApi(`/api/market/fish/${tradeId}/comments`, {
        method: 'POST',
  ...
  const handleCommentDelete = async (commentId: number) => {
    ...
    try {
      await fetchApi(`/api/market/fish/${tradeId}/comments/${commentId}`, {
        method: 'DELETE',
      });
      fetchComments();
    } catch (error) {
